...
module minirv (
    ...
    output wire invalid_inst//非法指令,,
);

    // SimpleBus状态机
    localparam CPU_IDLE = 8'd0; //发地址
    ...
    reg rom_arvalid; assign axi_cpu_rom_arvalid = rom_arvalid;// regwire 
    wire rom_arready;assign rom_arready = axi_rom_arready; // wire

    wire rom_rvalid;assign rom_rvalid = axi_rom_rvalid; //wire
    wire [31:0]rom_rdata; assign rom_rdata = axi_rom_rdata;//wire
    reg rom_rready; assign axi_cpu_rom_rready = rom_rready;// regwire

    // axi-lite信号（RAM 侧，功能后续实现）
    ...
    reg [31:0] ram_wdata; assign axi_ram_wdata = ram_wdata; assign ram_wdata = mem_write_data;
    reg [3:0] ram_wstrb; assign axi_ram_wstrb = ram_wstrb; assign ram_wstrb = mem_write_mask;
    ...
    reg [31:0]ram_awaddr; assign axi_ram_awaddr = ram_awaddr; assign ram_awaddr = mem_addr_nowordalign;
    reg ram_awvalid; assign axi_ram_awvalid = ram_awvalid;
    wire ram_awready; assign ram_awready = axi_ram_awready;
    reg global_en; //包括读写使能
    ...
    always @(posedge clk) begin
        ...
        if (rst) begin
            ...
            // // 同步写
            // if (mem_write_enable & global_en) begin
            //     // 写
            //     pmem_write(mem_addr_word_aligned, mem_write_data, {{4{1'b0}},mem_write_mask}, 1'b1);
            // end
    end
